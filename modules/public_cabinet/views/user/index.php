        ...
        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
            <div class="panel panel-default panel-gray">
                ...
                <div class="panel-body">
                    ...
                    <?php $form = ActiveForm::begin([
                        'id' => 'invoice-form',
                        'action' => ['/public_cabinet/user/invoice'],
                        'enableClientValidation' => true,
                        'validateOnSubmit' => true,
                        'options' => [
                            'target' => '_blank',
...
<script>
    ...
    $(document).on('click', '#do-disable-cameras', function () {
        ...
        if (trigger.hasClass('fa-toggle-on')) {
            $.post(yii.app.createUrl('public_cabinet/user/ajax-toggle-all-cameras'), {
    ...
    $(document).on('click', '#do-delete-all', function () {
        swal({
            ...
            $.post(yii.app.createUrl('public_cabinet/remove/delete-all'), {
                password: password
            }).done(function (response) {
                if (response == 'OK') {
    ...
    $(document).on('click', '#tariff-switch', function () {
        ...
        if (selectedRadio.length > 0) {
            $.post(yii.app.createUrl('public_cabinet/user/ajax-change-tariff'), {
    ...
    $(document).on('click', '#payment-btn', function (e) {
        ...
        if (amount !== '') {
            if ($('#robokassa').prop('checked')) {
                $.post(yii.app.createUrl('public_cabinet/user/ajax-payment-modal'), {
                    amount: amount
                }).done(function (response) {
